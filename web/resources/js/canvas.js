...
function drawFigure(ctx, r = 1) {
    ...
    ctx.lineTo(pd+(width-2*pd)/4, height / 2);
    ctx.lineTo(pd+(width-2*pd)/4, pd);
    ctx.lineTo(width/2, pd);
    ctx.lineTo(width/2, pd+(height-2*pd)/4);
    ctx.lineTo(width-pd, height / 2);
    ctx.lineTo(width/2, height / 2);
    ctx.arc(width / 2, height / 2,
        (width - 2 * pd) / 2, Math.PI/2, Math.PI);
    ctx.fill();
...
function drawPoint(ctx, check) {
    ...
    if (tr) {
        ...
        if (JSON.parse(check.hit)) {
            ...
            ctx.fillStyle = "#00cc15";
        } else {

            ctx.fillStyle = "#ff4615";
        }
    }
    ctx.beginPath();
